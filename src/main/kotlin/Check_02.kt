fun check_02_after_tps (s: Stmt) {
    ...
    fun fe (e: Expr) {
        when (e) {
            ...
            is Expr.Func -> {
                All_assert_tk(e.tk, funcs.contains(e) || e.type_.clo == null) {
                    ...
                    fun aux (tp: Type, dofunc: Boolean): Type {
                        return when (tp) {
                            ...
                            is Type.Ptr -> {
                                ...
                                if (ret == null) tp else {
                                    Type.Ptr(tp.tk_, ret, aux(tp.pln,dofunc)).build(e)
                                }
                            }
                            is Type.Func -> if (!dofunc) tp else {
                                ...
                                Type.Func(tp.tk_, ret, tp.scps, aux(tp.inp,dofunc), aux(tp.out,dofunc)).build(e)
                            }
                        }
                    }
                    Pair (
