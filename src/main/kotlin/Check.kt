...
fun Stmt.getDepth (drop: Boolean): Int {
    return if (this is Stmt.Var && this.outer) {
        this.env("arg")!!.getDepth(true)
    } else {
        //val env2 = if (!drop) env else env.dropWhile { it != this }
...
fun Expr.getDepth (caller: Int, hold: Boolean): Pair<Int,Stmt.Var?> {
    return when (this) {
        ...
        is Expr.Var -> {
            val dcl = this.env()!!
            return if (hold) Pair(dcl.getDepth(true), dcl) else Pair(0, null)
        }
        is Expr.Upref ->  {
...
fun check_pointers (S: Stmt) {
    fun fs (s: Stmt) {
        when (s) {
            is Stmt.Var -> {
                val dst_depth = s.getDepth(false)
            ...
            is Stmt.Set -> {
                val set_depth = s.getDepth(false)
