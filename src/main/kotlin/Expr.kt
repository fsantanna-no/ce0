...
fun Attr.toExpr (): Expr {
    return when (this) {
        ...
        is Attr.Nat   -> Expr.Nat(this.tk_, this.type_)
        is Attr.Dnref -> Expr.Dnref(this.tk_,this.ptr.toExpr()).let {
            it.ptr.up = it
            it
        }
        is Attr.TDisc -> Expr.TDisc(this.tk_,this.tup.toExpr()).let {
            it.tup.up = it
            it
        }
        is Attr.UDisc -> Expr.UDisc(this.tk_,this.uni.toExpr()).let {
            it.uni.up = it
            it
        }
    }.let {
