...
class TExec {
    ...
    fun f05_user_big () {
        val out = all("""
            ...
            set s = <.1 [_1,_2,_3,_4]>:<[_int,_int,_int,_int],_int,_int>
            output std /s
        """.trimIndent())
    ...
    fun i15_ptr_tup () {
        val out = all("""
            var x: _int; set x = _10
            var p: [_int,/_int @local]; set p = [_10,/x]
            var v: _int; set v = _20
            set p.2 = /v
            output std p.2\
        """.trimIndent())
    ...
    fun i19_ptr_tup () {
        val out = all("""
            ...
            var v: _int; set v = _20
            var x2: [_int,/_int @local]; set x2 = [_10,/v]
            set x1 = x2
            output std x1.2\
        """.trimIndent())
        assert(out == "20\n")
    ...
    fun j09_tup_list_err () {
        val out = all("""
            ...
            set t = [_10, new <.1 <.0>:/</^ @local> @local>:</^ @local> @local]
            output std /t
        """.trimIndent())
        assert(out == "[10,<.1 <.0>>]\n") { out }
    }
    ...
    fun m03_scope_f () {
        val out = all("""
            var f: [/</^@1>@1,/</^@3>@3]->()
            set f = func [/</^@1>@1,/</^@3>@3]->() {
                set arg.1\!1 = new <.1 <.0>:/</^@1>@1>:</^@1> @1
                set arg.2\!1 = new <.1 <.0>:/</^@3>@3>:</^@3> @3
            }
            {
                var x: /</^ @local> @local
                set x = new <.1 <.0>: /</^ @local> @local>:</^ @local> @local
