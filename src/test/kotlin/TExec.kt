...
class TExec {
    ...
    fun i20_ptr_uni_err () {
        val out = all("""
            ...
            var ptr: \_(char*); set ptr = borrow \uni!1
            set uni = <.2 _65>
            call _puts /ptr
        """.trimIndent())
    ...
    fun j09_list_move () {
        val out = all("""
            var l1: <?^>; set l1 = new <.1 <.0>>
            var l2: [_int,<?^>]; set l2 = [_10, replace l1=<.0>]
            output std l1
            output std l2.2
        """.trimIndent())
        assert(out == "<.0>\n<.1 <.0>>\n")
    ...
    fun j10_tup_copy_ok () {
        val out = all("""
            var l: <?^>; set l = <.0>
            var t1: [<?^>]; set t1 = [consume l]
            var t2: [<?^>]; set t2 = [copy t1.1]
            output std t2
        """.trimIndent())
        assert(out == "[<.0>]\n")
    }
    ...
    fun z12_union_tuple () {
        val out = all("""
            ...
            var s21: \<?[_int,^^]>; set s21 = borrow \tk2!3
            output std /s21
        """.trimIndent())
        assert(out == "<.0>\n")
    }
