...
class TExec {
    ...
    fun i03_ptr_func () {
        val out = all("""
            var f: [@1,/_int@1]->_int
            set f = func [@1,/_int@1]->_int { return arg.2\ }
            var g: [@1,/_int@1]->_int
            set g = f
            var x: _int
            set x = _10
    ...
    fun j11_rec_double () {
        val out = all("""
            var n: /<</^^ @local>> @local
            set n = new <.1 <.1 new <.1 <.1 <.0>>> :@local>>: @local
            output std n
        """.trimIndent())
    ...
    fun j09_tup_list_err () {
        val out = all("""
            ...
            set t = [_10:_int, new <.1 <.0>:/</^ @local> @local>:</^ @local>: @local]
            output std /t
        """.trimIndent())
        assert(out == "[10,<.1 <.0>>]\n") { out }
    }
    ...
    fun m03_scope_f () {
        val out = all("""
            var f: [@1,@3,/</^@1>@1,/</^@3>@3]->()
            set f = func [@1,@3,/</^@1>@1,/</^@3>@3]->() {
                set arg.3\!1 = new <.1 <.0>:/</^@1>@1>:</^@1>: @1
                set arg.4\!1 = new <.1 <.0>:/</^@3>@3>:</^@3>: @3
            }
            {
                var x: /</^ @local> @local
                set x = new <.1 <.0>: /</^ @local> @local>:</^ @local>: @local
    ...
    fun n03_pool () {
        val out = all("""
            var f : [@1,/_int@1] -> /()@1
            set f = func [@1,/_int@1] -> /()@1 {
                return arg.2
            }
            var x: _int
            call f [@local,/x]: @local
            output std ()
        """.trimIndent()
        )
        assert(out == "()\n") { out }
    ...
    fun z10_return_move () {
        val out = all("""
            var f: (@1)-><(),_int,/<[_int,/^@1]>@1>
            set f = func (@1)-><(),_int,/<[_int,/^@1]>@1> {
                var str: /<[_int,/^@1]> @1
                set str = <.0>: /<[_int,/^@1]> @1
                return <.3 str>:<(),_int,/<[_int,/^@1]>@1>
            }
            var x: <(),_int,/<[_int,/^@1]>@1>
            set x = call f (@local)
            output std x!3
