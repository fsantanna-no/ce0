...
class TExec {
    ...
    fun d07_func_fg () {
        val out = all("""
            var f:({}->{}->  _int->_int)
            set f = func {}->{}-> _int->_int { return arg }
            var g:  ({}->{@_1}-> [({}->{}-> _int->_int), _int] -> _int)
            set g = func ({}->{@_1}-> [({}->{}-> _int->_int), _int]-> _int) {
               var fx: ({}->{}-> _int->_int)
               set fx = arg.1
               var v: _int
               set v = call fx arg.2
    ...
    fun d08_func_unit () {
        val out = all("""
            var f: ({}->{}->  ()->() )
            set f = func {}->{}-> ()->() { var x:() ; set x = arg ; return arg}
            var x:() ; set x = call f ()
            output std x
        """.trimIndent())
        assert(out == "()\n") { out }
    ...
    fun i03_ptr_func () {
        val out = all("""
            ...
            set f = func{}-> {@_1}->/_int@_1->_int { return arg\ }
    ...
    fun i09_ptr_func_err () {
        ...
        """.trimIndent())
        assert(out == "(ln 4, col 5): invalid return : type mismatch") { out }
    }
    @Test
    ...
    fun n06_pool_ff() {
        val out = all(
            ...
            var g: ({}->{@_1}-> [ ({}->{@_1}-> /()@_1->/()@_1), /()@_1] -> /()@_1)
            set g = func {} ->{@_1}->[({}->{@_1}-> /()@_1->/()@_1), /()@_1] -> /()@_1 {
                return call arg.1 {@_1} arg.2: @_1
            }
            var x: ()
            output std call g {@local} [f,/_x]: @local
        """.trimIndent()
        )
        assert(out == "_\n") { out }
    }
    ...
    fun n10_pool_closure () {
        val out = all(
            ...
            set cnst = func {}->{@_1}-> /_int@_1 -> ({@_1}->{}-> () -> /_int@_1) {
                var x: /_int@_1
                set x = arg
