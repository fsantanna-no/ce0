...
class TEnv {
    ...
    fun b07_user_rec_up () {
        val out = inp2env("""
            var x: /_int @a
        """.trimIndent())
        assert(out == "(ln 1, col 14): undeclared scope \"@a1\"") { out }
    }
    @Test
    fun b09_user_err () {
        val out = inp2env("""
    ...
    fun b14_pool_err () {
        val out = inp2env("""
            ...
            output std l\!0
        """.trimIndent())
        assert(out == "(ln 2, col 55): undeclared scope \"@aaa1\"") { out }
    }
    @Test
    fun b15_pool_err () {
        val out = inp2env("""
    ...
    fun b16_pool_err () {
        ...
        assert(out == "(ln 1, col 24): expected expression : have `@aaa´") { out }
    }
    @Test
    fun b16_pool_err2 () {
        val out = inp2env("""
            call _f:$F {@aaa} ()
        """.trimIndent())
        assert(out == "(ln 1, col 25): undeclared scope \"@aaa1\"") { out }
    }
    @Test
    fun b17_pool_err () {
        val out = inp2env("""
            ...
            call f {} (): @aaa
        """.trimIndent())
        assert(out == "(ln 2, col 15): undeclared scope \"@aaa1\"") { out }
    }
    @Test
    fun b18_pool_err () {
        val out = inp2env("""
    ...
    fun e07_ptr_err4 () {
        val out = inp2env("""
            var f: func @a->{}->()->()
        """.trimIndent())
        assert(out == "(ln 1, col 13): undeclared scope \"@a1\"") { out }
    }
    @Test
    fun e07_ptr_err5 () {
        val out = inp2env("""
    ...
    fun m06_pred_notunion () {
        val out = inp2env("""
            ...
            call _f:$F l?1
        """.trimIndent())
        assert(out == "(ln 2, col 26): invalid discriminator : not an union") { out }
    }
    @Test
    ...
    fun p23_pool_closure_err2() {
        ...
        assert(out == "(ln 1, col 14): expected `}´ : have \"@\"") { out }
    }
    @Test
    ...
    fun p31_closure_err () {
        ...
        """.trimIndent())
        assert(out == "(ln 1, col 29): invalid function type : missing pool argument") { out }
    }
    @Test
    fun p32_test () {
        val out = inp2env(
